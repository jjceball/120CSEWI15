     ...
     char** argv;
{
  ...
  if (strncmp(argv[1], "0", 5) == 0) { // set vars using own funcs
//    Printf("USING YOUR KERNEL\n");
    ...
    sched = &MySchedThread;
  } else if (strncmp(argv[1], "1", 4) == 0) { // else use ref funcs
    init = &InitThreads;
    spawn = &SpawnThread;
    yield = &YieldThread;
    get = &GetThread;
    ...
    Printf ("Only arguments accepted are '0' (yours) or '1' (reference)\n");
    return;
  }
